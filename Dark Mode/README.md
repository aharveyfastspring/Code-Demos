...
- Switching themes re-points `data-storefront` at a different storefront (`embedded-light` / `embedded-dark`) and fully tears down and reinitializes the SBL script, since FastSpring reads `data-storefront` once at script load and doesn't support swapping it live.
